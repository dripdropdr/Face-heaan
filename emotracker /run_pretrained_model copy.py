...
MODEL_DIR = 'pytorch-benchmarks/models/'
...
class ResNet50():
    ...
    def load(self, model_path):
        self.resnet50.load_state_dict(torch.load(model_path, map_location=DEVICE))  
...
def main():
    ...
    if opt.eval_with_students:
        ...
        paths = sorted(glob.glob(os.path.join('pretrained_models', opt.model_type, '*.pth')))
        if len(paths) == 0:
            paths = glob.glob(os.path.join('Multitask-'+ opt.model_type, 'Seq_len={}'.format(opt.seq_len), '*student*'))

        print(os.path.join('Multitask-'+ opt.model_type, 'Seq_len={}'.format(opt.seq_len), '*student*'))
        print(paths)
        assert len(paths) != 0
